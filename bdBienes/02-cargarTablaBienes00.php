<?php	
	$salida.= '
		<td class="img sticky1" ></td>
		<td class="img sticky2" ></td>
...
if($mod[1]==1){ //Aquí se alerta una modificación.
	...
	$salida.= 
	'	<td id="tdBien'.$fila1["codBien"].'" class="sticky4" title="Inicie sesión para para modificar.">'.$fila1["nomBien"].'</td>';
}
//========================DETALLES DEL BIEN======================
...
if($mod[7]==1){ //Aquí se alerta una modificación.
	...
	$salida.= 
	'	<td id="tdCant'.$fila1["codBien"].'" class="sticky6" style="text-align:right;padding:0px 10px" title="Inicie sesión para para modificar.">'.$fila1["cantBien"].'</td>';
}
//========================ESTADO (SELECT)======================
...
if($mod[10]==1){//==== Aquí hay MODIFICACIONES PENDIENTES		
	...
	if($nomEstado=="MALO" && $nomMantenimiento!="DADO DE BAJA"){
		$salida.= '
			<td id="tdEstado'.$fila1["codBien"].'" class="sticky7" title="Este elemento esta malo, pero aún no ha sido dado de baja.&#13;&#13;Inicie sesión para para modificar.">'.$nomEstado.' <img onMouseOver="this.style.background=\'none\'; this.style.borderRadius=\'0px\'" style="width:10px; height:9px" src="../art/alerta.svg"/> Dar de baja. </td>';
	}else{	
		$salida.= '<td id="tdEstado'.$fila1["codBien"].'" class="sticky7" title="Inicie sesión para para modificar.">'.$nomEstado.'</td>';
	}
}
