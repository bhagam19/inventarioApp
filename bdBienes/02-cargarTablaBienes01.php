<?php 
	//error_reporting(0);
	$salida.= '
			<td class="img sticky1" ></td><!-- <img title = "Transferir este bien a otro usuario." src="../art/transferir2.svg" ondblclick=""/> --> 
			<td class="img sticky2" ></td><!--<img title ="Editar este bien." src="../art/editar.svg" ondblclick=""/>-->
			<td class="sticky3" style="text-align:center">'.$fila1["codBien"].'</td>
		';	
//========================NOMBIEN======================
if($mod[1]==1){ //Aquí se alerta una modificación.
	$salida.= 
	'	<td id="tdBien'.$fila1["codBien"].'" class="sticky4" style="background:#D2F6B3; border: 2px solid gray"  title="Valor original: '.$fila1["nomBien"].'.&#13;&#13;Este cambio está pendiente de ser verificado y aprobado por el administrador." ondblclick="actualizarInputBien(this.id,'.$fila1["codBien"].',\'nomBien\',\'bienAct'.$fila1["codBien"].'\',\''.$queryUrl.'\',\'95px\',event)"><img onMouseOver="this.style.background=\'none\'; this.style.borderRadius=\'0px\'" style="width:10px; height:10px" src="../art/modificar.svg"/> '.$vlr[1].'</td>';
}else{
	$salida.= 
	'	<td id="tdBien'.$fila1["codBien"].'"class="sticky4" ><img style="width:10x;height:10x" title="Click para modificar" src="../art/editar.png" onclick="actualizarInputBien(this.parentNode.id,'.$fila1["codBien"].',\'nomBien\',\'bienAct'.$fila1["codBien"].'\',\''.$queryUrl.'\',\'95px\',event)">'.$fila1["nomBien"].'</td>';
}	
//========================DETALLES DEL BIEN======================
...
//========================ESTADO (SELECT)======================
	...
	if($mod[10]==1){//==== Aquí hay MODIFICACIONES PENDIENTES
		...
		if($nomEstado=="MALO" && $nomMantenimiento!="DADO DE BAJA"){
			...
			$salida.= '<td id="tdEstado'.$fila1["codBien"].'"class="sticky7" ><img style="width:10x;height:10x" title="Click para modificar" src="../art/editar.png" onclick="actualizarSeleccionBien(this.parentNode.id,'.$fila1["codBien"].',\'codEstado\',\'estadoAct'.$fila1["codBien"].'\','.$fila1['codEstado'].',\'estadoDelBien\',\'nomEstado\',\''.$queryUrl.'\',\'80px\')">'.$nomEstado.'</td>';
		}
	}
